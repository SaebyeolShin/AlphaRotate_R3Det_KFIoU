...
from alpharotate.libs.models.backbones.mobilenet.mobilenet_v2 import op
from alpharotate.libs.models.backbones.mobilenet.mobilenet_v2 import ops
...
class MobileNetV2Backbone(object):
    ...
    def __init__(self, cfgs):
        self.cfgs = cfgs
        self.expand_input = ops.expand_input_by_factor

        self.V2_BASE_DEF = dict(
            defaults={
                ...
                (ops.expanded_conv,): {
                    'expansion_size': self.expand_input(6),
                    'split_expansion': 1,
            ...
            spec=[
                ...
                op(ops.expanded_conv,
                   expansion_size=self.expand_input(1, divisible_by=1),
    ...
    def mobilenetv2_scope(self, is_training=True,
                          trainable=True,
    ...
    def mobilenetv2_base(self, img_batch, is_training=True):

        with slim.arg_scope(self.mobilenetv2_scope(is_training=is_training, trainable=True)):
            feature_to_crop, endpoints = mobilenet_v2.mobilenet_base(input_tensor=img_batch,
                                                                     num_classes=None,
                                                                     is_training=False,
                                                                     depth_multiplier=1.0,
                                                                     scope='MobilenetV2',
                                                                     conv_defs=self.V2_BASE_DEF,
    ...
    def mobilenetv2_head(self, inputs, is_training=True):
        with slim.arg_scope(self.mobilenetv2_scope(is_training=is_training, trainable=True)):
